  ...
  public:
    ...
    void receive(int)
    {
        ...
        if (waiting_for_response_) {
            timers_->schedule(&request_deadline_timer_, pace_interval_ticks_ * RESPONSE_SIZE * 2);
    ...
    void on_request()
    {
        if (!closing_) {
            ...
            timers_->schedule(&request_deadline_timer_, pace_interval_ticks_ * RESPONSE_SIZE * 4);
    ...
    void unidle()
    {
        if (allow_schedule_in_range) {
            timers_->schedule_in_range(&idle_timer_, 60 * time_s, 61 * time_s);
        } else {
...
int main(int argc, char **argv)
{
    ...
    printf("%s,%d,%s,%lf,%ld\n", argv[0], pair_count, (allow_schedule_in_range ? "yes" : "no"),
