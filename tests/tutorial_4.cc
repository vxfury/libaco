...
static void co_fp0()
{
    ...
    // you should always call `aco_exit()` to finish the execution of a non-main
    // co instead of call `return` in the real application this is a demo shows
    // how protector works in libaco (intended to abort)
    return;
    aco_exit();
    aco_assert(0);
}
